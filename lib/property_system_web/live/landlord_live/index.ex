defmodule PropertySystemWeb.LandlordLive.Index do
  ...
  def mount(_params, session, socket) do
    ...
    propertys =
      ...
      |> Enum.filter(fn property -> property.user_id == landlord.id end)

    property_ids = propertys |> Enum.map(& &1.id)

    tenants =
      Tenants.list_tenants() |> Enum.filter(fn tenant -> tenant.property_id in property_ids end)

    manager_ids = propertys |> Enum.map(& &1.manager_id)

    managers =
      Enum.map(manager_ids, fn manager_id ->
